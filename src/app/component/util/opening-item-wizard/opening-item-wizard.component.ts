...
@Component({
  selector: 'app-opening-item-wizard',
  templateUrl: './opening-item-wizard.component.html',
  styleUrls: ['./opening-item-wizard.component.scss'],
  imports: [
    FormsModule,
    TranslateModule,
    NgIf,
...
export class OpeningItemWizardComponent extends MySubscribable implements OnInit {
    ...
    public ngOnInit() {
        ...
        for (let i = 0; i < openingDayRanges.length; i++) {
            let from = Time.toString(openingDayRanges[i].from);
            let till = Time.toString(openingDayRanges[i].till);
    ...
    public addNewOpeningDayRange(): void {
        ...
        // let openingDayRange: OpeningDayRange = openingDayRanges[openingDayRanges.length];
        let openingDayRange: OpeningDayRange = new OpeningDayRange();
